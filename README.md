...
> 이 글은 Tom O'Reilly가 2003년 Warburg-Pincus' 과학 기술대회에서 발언을 기반으로 작성되었다.

* 1962년 Thomas Kuhn은 The Structure of Scientific Revolutions라는 책에서 과학이 점진적으로 진보하는 것이 아니고, 코페르니쿠스의 태양 중심설, 다윈의 진화론처럼 획기적인 성과가 나오는 순간에 폭발적으로 발전하는 것이라고 주장했는데, 이와 같은 과학의 혁명적인 과정을 **paradigm shifts**라고 한다.

* 과학계와 같은 이런 혁명적인 변화가 상업계에도 많이 발생하는데, 그러한 패러다임 변화 중 하나는 1981년에 IBM 개인용 컴퓨터의 표준화 된 아키텍처가 도입됨에 따라 발생했다.
그 당시에 소프트웨어는 주로 지정된 하드웨어 플랫폼에서만 실행할 수 있어서 거의 단독 판매하지 않았기 때문에 새로운 시스템에 운영 체제를 제공 할 시점에 IBM은 Microsoft라는 소규모 회사에 라이센스를 줘서, IBM이 통제하지 않은 소규모 시장에 소프트웨어를 재판매 할 권리를 포기하기로 결정했다. 
그러나 소프트웨어가 향후에 미칠 영양을 예측하지 못한 IBM은 새로운 시장에서 리더십을 잃어버리고 Microsoft가 컴퓨터 업계에서 가장 중요한 회사가되었다.

* 또 하나의 사례로 개인용 컴퓨터의 초기 경쟁에서 PC시장의 리더들은 모두 디바이스 성능을 향상시키기 위해 경쟁했는데, Michael Dell은 개인용 컴퓨터 시장의 포인트는 더 좋은 성능이 아니라 생산 비용을 낮추면서 충분히 좋은 디바이스를 만드는 거라고 생각했고 동시에 Intel은 메모리 칩 사업을 포기하고 CPU 사업을 시작했다.  
결과는 새로운 상품 플랫폼을 받아 들인 Dell과 Intel이 승자가 되었다.

* 거의 모든 사람이 구글을 사용하고 있지만, 20~80%의 사용자만 자신이 리눅스를 사용하고 있다고 답했다. 그런데 구글은 10만대 이상의 리눅스 서버를 사용하고 있다.  대부분의 사람들은 눈앞에 있는 것만 유의한다. 우리가 오픈소스의 진정한 장기적인 paradigm shift를 이해하고 싶다면, 표면을 투과하고 이 혁명의 원인과 결과 모두를 더 깊게 이해해야 한다.

* 오픈 소스는 세가지 깊고 장기적인 트렌드로 설명하는 것이 유용하다:
  - 소프트웨어의 상품화  
  - 네트워크를 사용한 협업  
  - 사용자 정의 가능한 소프트웨어(서비스로서의 소프트웨어)  

## 상품으로서의 소프트웨어
* ‘상품’(commodity)이라는 단어는 다양한 목적을 위한 기본 구성 요소로 가치가 있는 사물 또는 물질이라는 뜻인데, 상품은 항상 둘 이상의 생산자가 제공하며 소비자는 한 생산자의 제품을 다른 생산자의 제품으로 대체가 가능하기 때문에 생산자들은 그들이 준수해야하는 통일 된 품질 기준이 있습니다. 

* 소프트웨어 상품도 마찬가지입니다. 예를 들면, 인터넷은 표준적인 공유 프로토콜 위에 마련되서 어떤 서버든, 어떤 브라우저든 상관없이 다 호환 가능합니다.  

* ‘상품’(commodity)은 표준화된 방식에 의해 생산되는 물건을 의미하고, 한 상품은 다른 비슷한 상품으로 쉽게 대체될 수 있습니다.
