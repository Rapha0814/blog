...
그 당시에 소프트웨어는 주로 지정된 하드웨어 플랫폼에서만 실행할 수 있어서 거의 단독 판매하지 않았습니다. 그래서 새로운 시스템에 운영 체제를 제공 할 시점에 IBM은 Microsoft라는 소규모 회사에 라이센스를 줘서, IBM이 통제하지 않은 소규모 시장에 소프트웨어를 재판매 할 권리를 포기하기로 결정했습니다. 
그러나 소프트웨어가 향후에 미칠 영양을 예측하지 못한 IBM은 새로운 시장에서 리더십을 잃어버리고 Microsoft가 컴퓨터 업계에서 가장 중요한 회사가되었습니다.

* 또 하나의 사례로 개인용 컴퓨터의 초기 경쟁에서 PC시장의 리더들은 모두 디바이스 성능을 향상시키기 위해 경쟁했는데, Michael Dell은 개인용 컴퓨터 시장의 포인트는 더 좋은 성능이 아니라 생산 비용을 낮추면서 충분히 좋은 디바이스를 만드는 거라고 생각했고 동시에 Intel은 메모리 칩 사업을 포기하고 CPU 사업을 시작했습니다.  
결과는 새로운 상품 플랫폼을 받아 들인 Dell과 Intel이 승자가 되었습니다.

* 거의 모든 사람이 구글을 사용하고 있지만, 20~80%의 사용자만 자신이 리눅스를 사용하고 있다고 답했습니다. 그런데 구글은 10만대 이상의 리눅스 서버를 사용하고 있습니다.  대부분의 사람들은 눈앞에 있는 것만 유의합니다. 우리가 오픈소스의 진정한 장기적인 paradigm shift를 이해하고 싶다면, 표면을 투과하고 이 혁명의 원인과 결과 모두를 더 깊게 이해해야 합니다.

* 오픈 소스는 세가지 깊고 장기적인 트렌드로 설명하는 것이 유용합니다:
  - 소프트웨어의 상품화  
  - 네트워크를 사용한 협업  
